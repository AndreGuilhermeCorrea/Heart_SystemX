...
//classe implementa interface SaudeDAO, vai encapsular todas operações referente ao banco de dados.
public class SaudeDaoJDBC implements SaudeDAO {
	...
	public void insert(Saude objSaude) {
		...
		try {
			...
			pst.setString(20, objSaude.getStend());
	...
	public void update(Saude objSaude) {
		...
		String campos = "cpf=?, data=?, tabagista=?, etilista=?, "
				...
				+ "patologiaCardiaca=?, arritmia=?, infarto=?, stend=?, cateterismo=?, aVC=?, renal=?, cancer=?, "
	...
	private Saude instanciaSaude(ResultSet rs) throws SQLException {
		...
		objSaude.setStend(rs.getString("stend"));
